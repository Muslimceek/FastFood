...
import { Product } from '../../types'; // Assuming types exist, extended below
import { 
  X, Plus, Minus, Star, Check, Info, 
  ChevronDown, Flame, Wheat, AlertCircle 
} from 'lucide-react';

// --- EXTENDED TYPES (For Production Realism) ---
// In a real app, these come from the backend. 
// We mock them here to demonstrate the complex logic.
...
interface ProductModalProps {
  product: Product;
  onClose: () => void;
  onAddToCart: (item: any) => void; // Passing full config object
}
...
export const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onAddToCart }) => {
  ...
  // Animation ref for mounting
  const contentRef = useRef<HTMLDivElement>(null);

  // Initialize Default Selections (e.g. select first option if required & single)
  useEffect(() => {
  ...
  const handleToggleOption = (groupId: string, optionId: string, type: ModifierType) => {
    ...
    setSelections(prev => {
      ...
      if (type === 'single') {
        // Radio behavior: replace entire array with new ID
        return { ...prev, [groupId]: [optionId] };
      } else {
        // Checkbox behavior: toggle ID
  ...
  const handleSubmit = () => {
    if (!isValid) return; // Should be handled by disabled button, but double check
    ...
    onAddToCart(finalConfiguration); // Pass explicit config
    handleClose();
  };
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4 transition-all duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div 
        ref={contentRef}
        className={`
          relative bg-white w-full sm:max-w-[480px] h-[92vh] sm:h-auto sm:max-h-[90vh] 
          rounded-t-[32px] sm:rounded-[32px] flex flex-col shadow-2xl overflow-hidden 
          transform transition-transform duration-300 ease-out
          ${isClosing ? 'translate-y-full' : 'translate-y-0'}
        `}
      >
        
        {/* --- 1. HERO IMAGE (Parallax-like) --- */}
        <div className="relative h-64 sm:h-72 bg-slate-100 shrink-0 group">
            ...
            <button 
                onClick={handleClose}
                className="absolute top-4 right-4 bg-white/20 backdrop-blur-md p-2.5 rounded-full text-white hover:bg-white hover:text-slate-900 transition-all active:scale-90 z-20"
            >
                <X size={20} strokeWidth={2.5} />
            </button>
            
            {/* Title Overlay (Competitor Style) */}
            ...
            <div className="p-6 space-y-8 pb-32">
                ...
                <section className="space-y-6">
                  ...
                  <div className="flex gap-3">
                      <NutrientPill 
                        value={product.calories || 250} 
                        label="ккал" 
                        icon={<Flame size={14} className="text-orange-500" fill="currentColor"/>} 
                      />
                      <NutrientPill 
                        value={product.weight || '300 г'} 
                        label="вес" 
                        icon={<Info size={14} className="text-blue-500"/>} 
                      />
                      <NutrientPill 
                        value={(product.nutrients?.proteins || 12) + ' г'} 
                        label="белки" 
                        icon={<Wheat size={14} className="text-yellow-500"/>}
                      />
                  </div>
                  
                  {/* Allergens Warning (If needed) */}
                  <div className="flex items-start gap-2 bg-yellow-50 p-3 rounded-xl text-xs text-yellow-800 border border-yellow-100">
                    ...
                    <span>Содержит аллергены: глютен, лактоза. Пожалуйста, сообщите нам об ограничениях.</span>
                  </div>
                </section>
                           ...
                           return (
                             <div 
                               ...
                               className={`
                                 ...
                                 transition-all duration-200 select-none active:scale-[0.99]
                                 ${isSelected 
                             ...
                             >
                                <div className="flex items-center gap-4">
                                   ...
                                   <div>
                                     <span className={`block text-sm font-bold ${isSelected ? 'text-slate-900' : 'text-slate-700'}`}>
                                       {option.name}
                                     </span>
                                     {option.calories && (
                                       <span className="text-[10px] text-slate-400 font-medium">
                                         {option.calories} ккал
                                       </span>
                                     )}
                                   </div>
                                </div>
                                ...
                                {option.price > 0 ? (
                                  ...
                                  <span className="text-xs font-bold text-slate-400 uppercase">Бесплатно</span>
                                )}
                             </div>
                           );
                         })}
                ...
                {/* Special Instructions (Competitor feature) */}
                <section>
            ...
            <div className="flex items-center gap-4 max-w-[480px] mx-auto">
                ...
                <div className="flex items-center bg-slate-100 rounded-xl p-1 h-14 shrink-0">
                    <button 
                        ...
                        className={`w-12 h-full flex items-center justify-center rounded-lg transition-colors ${quantity === 1 ? 'text-slate-300' : 'text-slate-900 hover:bg-white shadow-sm'}`}
                        disabled={quantity === 1}
                    >
                        <Minus size={20} />
                    </button>
                    ...
                    <button 
                        ...
                        className="w-12 h-full flex items-center justify-center rounded-lg text-slate-900 hover:bg-white shadow-sm transition-colors"
                    >
                        <Plus size={20} />
                    </button>
                </div>
                ...
                <button 
                    ...
                    disabled={!isValid}
                    className={`
                      flex-1 h-14 rounded-2xl font-bold text-lg flex items-center justify-between px-6 
                ...
                >
                    ...
                    <span>[Image of shopping cart icon] {calculateTotal} ₽</span>
                </button>
            </div>
